...
if(categoryId) params.append("blogpost_categories.id", categoryId.toString());

const response = await fetch(`${API_BASE_URL}/blogposts?${params.toString()}`);
...
if(params?.categoryId) query.append("blogpost_categories.id", params.categoryId.toString());
    const response = await fetch(`${API_BASE_URL}/blogposts/count${query.toString()}`);
